...
<xsl:stylesheet xmlns:xs="http://www.w3.org/2001/XMLSchema"
                ...
                xmlns:esp-d2e67244="http://www.example.org/cannot/really/use/XInclude"
                xmlns:esp-d2e67298="http://www.example.org/cannot/really/use/XInclude"
...
should correspond to an existing module, via a moduleSpec or
   ...
   <xsl:template match="tei:moduleRef[@key eq 'linking']" priority="1005" mode="M36">
      ...
      <xsl:variable name="except" select="tokenize( normalize-space(@except), ' ')"/>
		    ...
		    <!--ASSERT -->
      <xsl:choose>
         <xsl:when test="every $gi in $include satisfies $gi = ( 'link', 'linkGrp', 'ab', 'anchor', 'seg', 'when', 'timeline', 'join', 'joinGrp', 'alt', 'altGrp', 'standOff' )"/>
         <xsl:otherwise>
            <xsl:message>One or more of the elements included on the '<xsl:text/>
               <xsl:value-of select="@key"/>
               <xsl:text/>' ＜moduleRef＞ are not actually available in that module (every $gi in $include satisfies $gi = ( 'link', 'linkGrp', 'ab', 'anchor', 'seg', 'when', 'timeline', 'join', 'joinGrp', 'alt', 'altGrp', 'standOff' ))</xsl:message>
         </xsl:otherwise>
      </xsl:choose>

		    <!--ASSERT -->
      <xsl:choose>
         <xsl:when test="every $gi in $except  satisfies $gi = ( 'link', 'linkGrp', 'ab', 'anchor', 'seg', 'when', 'timeline', 'join', 'joinGrp', 'alt', 'altGrp', 'standOff' )"/>
         <xsl:otherwise>
            <xsl:message>One or more of the elements excepted on the '<xsl:text/>
               <xsl:value-of select="@key"/>
               <xsl:text/>' ＜moduleRef＞ are not actually available in that module (every $gi in $except satisfies $gi = ( 'link', 'linkGrp', 'ab', 'anchor', 'seg', 'when', 'timeline', 'join', 'joinGrp', 'alt', 'altGrp', 'standOff' ))</xsl:message>
         </xsl:otherwise>
      </xsl:choose>
      <xsl:apply-templates select="*" mode="M36"/>
   </xsl:template>
...
Child elements of <xsl:text/>
   ...
   <!--PATTERN schematron-constraint-tei_customization-mode-child-sanity-56-->


	  <!--RULE -->
   <xsl:template match="*[ @mode eq 'delete' ]" priority="1001" mode="M63">
   ...
   <!--PATTERN schematron-constraint-tei_customization-tei-source-57-->
