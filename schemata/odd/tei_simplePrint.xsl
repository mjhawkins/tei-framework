...
The element indicated by @spanTo (<xsl:text/>
   ...
   <!--PATTERN schematron-constraint-tei_simplePrint-pb-pbposition-27-->
   ...
   <xsl:template match="tei:pb" priority="1000" mode="M30">
      ...
      <xsl:if test="parent::*/text() and not           (preceding-sibling::text() and           following-sibling::text())">
         <xsl:message>please make sure pb elements are not at the start or end of mixed content
                   (parent::*/text() and not (preceding-sibling::text() and following-sibling::text()))</xsl:message>
      </xsl:if>
      <xsl:apply-templates select="*" mode="M30"/>
   </xsl:template>
   ...
   <!--PATTERN schematron-constraint-tei_simplePrint-ref-refAtts-28-->


	  <!--RULE -->
   <xsl:template match="tei:ref" priority="1000" mode="M31">
   ...
   <!--PATTERN schematron-constraint-tei_simplePrint-relatedItem-targetorcontent1-29-->
...
relatedItem element must be empty (@target and count( child::* ) &gt; 0)</xsl:message>
   ...
   <!--PATTERN schematron-constraint-tei_simplePrint-s-noNestedS-30-->


	  <!--RULE -->
   <xsl:template match="tei:s" priority="1000" mode="M33">
   ...
   <!--PATTERN schematron-constraint-tei_simplePrint-subst-substContents1-31-->


	  <!--RULE -->
   <xsl:template match="tei:subst" priority="1000" mode="M34">
   ...
   <!--PATTERN schematron-constraint-tei_simplePrint-text-headeronlyelement-34-->


	  <!--RULE -->
   <xsl:template match="tei:term | tei:biblFull " priority="1000" mode="M35">
   ...
   <!--PATTERN -->
	  ...
	  <!--RULE -->
   <xsl:template match="tei:glyphName" priority="1000" mode="M36">
   ...
   <!--PATTERN -->
	  ...
	  <!--RULE -->
   <xsl:template match="tei:charName" priority="1000" mode="M37">
